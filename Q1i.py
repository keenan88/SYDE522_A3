...
if __name__ == "__main__":
    ...
    discussion = \
    """
         The scaling does reduce the RMSE, though perhaps artificially.
         The scaler is observable to really just cluster
         the data more tightly, but about the origin, not about each cluster.
         An actually useful scaling would be scaling each dataset down about their own 
         mean, to reduce overlap between the two datasets.
         
         Since these two datasets have lots of little 1-off random overlap, it is much harder
         for the model to find a linearly seperable solution in high dimensonal space.
